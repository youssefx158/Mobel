  ...
  const SERVER_VERSION = "20260505-4";
  ...
  server.listen(config.port, "0.0.0.0", () => {
    ...
    console.log("==================================================");
    console.log(`  MD Store Server  •  version ${SERVER_VERSION}`);
    console.log("==================================================");
    ...
    if (DATA_DIR.startsWith(tmpRoot)) {
      console.warn("⚠️  WARNING: Data directory is inside the OS temp folder.");
      console.warn("    Files here WILL BE LOST after a container restart.");
      console.warn("    Make sure the application root has write permissions, or set DATA_DIR to a persistent path.");
    }
    if (UPLOADS_DIR.startsWith(tmpRoot)) {
      console.warn("⚠️  WARNING: Uploads directory is inside the OS temp folder.");
      console.warn("    Uploaded images will disappear after a restart.");
      console.warn("    Make sure UPLOADS_DIR points to a persistent path.");
    }
  });
    ...
    try {
      ...
      if (url.pathname === "/api/health") {
        ...
        return sendJson(res, 200, {
          ...
          storage: {
            ...
            dataPersistent: !DATA_DIR.startsWith(tmpRoot),
            uploadsPersistent: !UPLOADS_DIR.startsWith(tmpRoot),
          },
        });
      }
      ...
      if (url.pathname === "/api/debug/routes") {
        return sendJson(res, 200, {
          ...
          // Routes confirmed to exist in this build:
          routes: [
            "GET /api/admin/custom-design-settings",
            "POST /api/admin/custom-design-settings",
